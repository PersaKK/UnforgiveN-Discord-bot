...
if(client.veritabanı.varMı("tag-" + member.guild.id) == true) {
	var tagid = client.veritabanı.veri("tag-" + member.guild.id)
	member.setNickname(tagid)
}
if(client.veritabanı.varMı("giris_" + member.guild.id) == true) {
	var kanalıd  = client.veritabanı.veri("giris_" + member.guild.id)
client.channels.get(kanalıd).send(`:inbox_tray: | <@${member.id}> adlı kişi sunucuya giriş yaptı.Seninle birlikte ${message.guild.memberCount} kişiyiz!`)
